...
# state indices
const STATE1_IDX1 = 1:HDIM_ISO
const STATE2_IDX1 = STATE1_IDX1[end] + 1:STATE1_IDX1[end] + HDIM_ISO
const INTCONTROLS_IDX1 = STATE2_IDX1[end] + 1:STATE2_IDX1[end] + CONTROL_COUNT
const CONTROLS_IDX1 = INTCONTROLS_IDX1[end] + 1:INTCONTROLS_IDX1[end] + CONTROL_COUNT
const DCONTROLS_IDX1 = CONTROLS_IDX1[end] + 1:CONTROLS_IDX1[end] + CONTROL_COUNT
const S1_IDX = DCONTROLS_IDX1[end] + 1:DCONTROLS_IDX1[end] + HDIM_ISO
...
# control indices
const D2CONTROLS_IDX1 = 1:CONTROL_COUNT

# model
struct Model2 <: AbstractModel
    h0_samples::Vector{SMatrix{HDIM_ISO, HDIM_ISO}}
end
@inline RD.state_dim(::Model2) = ASTATE_SIZE
@inline RD.control_dim(::Model2) = ACONTROL_SIZE
...
# dynamics
function RD.discrete_dynamics(::Type{RD.RK3}, model::Model2, astate::StaticVector,
                              acontrols::StaticVector, time::Real, dt::Real) where {SC}
    negi_hc = astate[CONTROLS_IDX1[1]] * NEGI_H1_ISO
    h_prop = exp((FQ_NEGI_H0_ISO + negi_hc) * dt)
    state1 = h_prop * astate[STATE1_IDX1]
    state2 = h_prop * astate[STATE2_IDX1]
    intcontrols = astate[INTCONTROLS_IDX1[1]] + dt * astate[CONTROLS_IDX1[1]]
    controls = astate[CONTROLS_IDX1[1]] + dt * astate[DCONTROLS_IDX1[1]]
    dcontrols = astate[DCONTROLS_IDX1[1]] + dt * acontrols[D2CONTROLS_IDX1[1]]
...
function run_traj(;gate_type=zpiby2, evolution_time=18., solver_type=altro,
    ...
    target_astate_constraint = GoalConstraint(xf, [STATE1_IDX1; STATE2_IDX1; INTCONTROLS_IDX1])
    # must obey unit norm.
    norm_constraints = [NormConstraint(n, m, 1, TO.Equality(), idxs) for idxs in (
        STATE1_IDX1, STATE2_IDX1, S1_IDX, S2_IDX, S3_IDX,
    ...
    cidx_arr = Array(CONTROLS_IDX1)
    d2cidx_arr = Array(D2CONTROLS_IDX1)
