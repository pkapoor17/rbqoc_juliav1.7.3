...
const STATE1_IDX2 = SVector{HDIM_ISO}(1:HDIM_ISO)
const STATE2_IDX2 = SVector{HDIM_ISO}(STATE1_IDX2[end] + 1:STATE1_IDX2[end] + HDIM_ISO)
const INTCONTROLS_IDX2 = SVector{CONTROL_COUNT}(STATE2_IDX2[end] + 1:STATE2_IDX2[end] + CONTROL_COUNT)
const CONTROLS_IDX2 = SVector{CONTROL_COUNT}(INTCONTROLS_IDX2[end]
                                            + 1:INTCONTROLS_IDX2[end] + CONTROL_COUNT)
const DCONTROLS_IDX2 = SVector{CONTROL_COUNT}(CONTROLS_IDX2[end] + 1:CONTROLS_IDX2[end] + CONTROL_COUNT)
const DSTATE1_IDX2 = SVector{HDIM_ISO}(DCONTROLS_IDX2[end] + 1:DCONTROLS_IDX2[end] + HDIM_ISO)
const D2STATE1_IDX2 = SVector{HDIM_ISO}(DSTATE1_IDX2[end] + 1:DSTATE1_IDX2[end] + HDIM_ISO)
...
function RD.discrete_dynamics(::Type{RK3}, model::Model{DO}, astate::SVector,
    ...
    negi_h = (
        FQ_NEGI_H0_ISO
        + astate[CONTROLS_IDX2[1]] * NEGI_H1_ISO
    )
    h_prop = exp(negi_h * dt)
    state1_ = astate[STATE1_IDX2]
    state2_ = astate[STATE2_IDX2]
    state1 =  h_prop * state1_
    state2 = h_prop * state2_
    intcontrols = astate[INTCONTROLS_IDX2] + astate[CONTROLS_IDX2] * dt
    controls = astate[CONTROLS_IDX2] + astate[DCONTROLS_IDX2] * dt
    dcontrols = astate[DCONTROLS_IDX2] + acontrol[D2CONTROLS_IDX2] * dt
    ...
    if DO >= 2
        d2state1_ = astate[D2STATE1_IDX2]
...
function run_traj(;gate_type=xpiby2, evolution_time=56.8, solver_type=altro,
                ...
                # for terminal  run_traj(;gate_type=xpiby2, evolution_time=56.8, solver_type=altro, sqrtbp=false, derivative_order=0, integrator_type=rk3, qs=ones(7), smoke_test=false, dt_inv=Int64(1e1), constraint_tol=1e-8, al_tol=1e-4, pn_steps=2, max_penalty=1e11, verbose=true, save=true, max_iterations=Int64(2e5))
    # model configuration
    ...
    x0[STATE1_IDX2] = IS1_ISO_
    x0[STATE2_IDX2] = IS2_ISO_
    # x0[STATE3_IDX] = IS3_ISO_
    # x0[STATE4_IDX] = IS4_ISO_
    xf[STATE1_IDX2] = gate * IS1_ISO_
    xf[STATE2_IDX2] = gate * IS2_ISO_
    ...
    target_astate_constraint = GoalConstraint(xf, [STATE1_IDX2; STATE2_IDX2]) # ; INTCONTROLS_IDX2])
    # must obey unit norm.
    norm_idxs = [STATE1_IDX2, STATE2_IDX2]
