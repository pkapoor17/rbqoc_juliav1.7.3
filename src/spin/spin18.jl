...
# state indices
const STATE1_IDX3 = 1:HDIM_ISO                                                               #all the state indices showing error
const STATE2_IDX3 = STATE1_IDX3[end] + 1:STATE1_IDX3[end] + HDIM_ISO
const INTCONTROLS_IDX = STATE2_IDX3[end] + 1:STATE2_IDX3[end] + CONTROL_COUNT
...
# model
struct Model3 <: AbstractModel
    namp::Float64
end
@inline RD.state_dim(::Model3) = ASTATE_SIZE
@inline RD.control_dim(::Model3) = ACONTROL_SIZE
...
# dynamics
function RD.discrete_dynamics(::Type{RD.RK3}, model::Model3, astate::StaticVector,
    ...
    state1 = h_prop * astate[STATE1_IDX3]
    state2 = h_prop * astate[STATE2_IDX3]
...
function run_traj(;gate_type=xpiby2, evolution_time=56.8, solver_type=altro,
    ...
    target_astate_constraint = GoalConstraint(xf, [STATE1_IDX3; STATE2_IDX3; INTCONTROLS_IDX])
    # must obey unit norm
    norm_constraints = [NormConstraint(n, m, 1, TO.Equality(), idxs) for idxs in (
        STATE1_IDX3, STATE2_IDX3, S1_IDX, S2_IDX, S3_IDX, S4_IDX, S5_IDX, S6_IDX, S7_IDX, S8_IDX,
    )]
