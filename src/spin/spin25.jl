...
# state indices
const STATE1_IDX4 = SVector{HDIM_ISO}(1:HDIM_ISO)
const STATE2_IDX4 = SVector{HDIM_ISO}(STATE1_IDX4[end] + 1:STATE1_IDX4[end] + HDIM_ISO)
...
const INTCONTROLS_IDX4 = SVector{CONTROL_COUNT}(STATE2_IDX4[end] + 1:STATE2_IDX4[end] + CONTROL_COUNT)
const CONTROLS_IDX4 = SVector{CONTROL_COUNT}(INTCONTROLS_IDX4[end] + 1:
                                            INTCONTROLS_IDX4[end] + CONTROL_COUNT)
const DCONTROLS_IDX4 = SVector{CONTROL_COUNT}(CONTROLS_IDX4[end] + 1:
                                             CONTROLS_IDX4[end] + CONTROL_COUNT)
const PFIRX_IDX = SVector{PFIR_SIZE_BY2}(DCONTROLS_IDX4[end] + 1:DCONTROLS_IDX4[end] + PFIR_SIZE_BY2)
const PFIRY_IDX = SVector{PFIR_SIZE_BY2}(PFIRX_IDX[end] + 1:PFIRX_IDX[end] + PFIR_SIZE_BY2)
# control indices
const D2CONTROLS_IDX4 = SVector{CONTROL_COUNT}(1:CONTROL_COUNT)
# sample indices
const S1_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 0 + 1:HDIM_ISO * 1)
const S2_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 1 + 1:HDIM_ISO * 2)
const S3_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 2 + 1:HDIM_ISO * 3)
const S4_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 3 + 1:HDIM_ISO * 4)
const S5_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 4 + 1:HDIM_ISO * 5)
const S6_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 5 + 1:HDIM_ISO * 6)
const S7_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 6 + 1:HDIM_ISO * 7)
const S8_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 7 + 1:HDIM_ISO * 8)
const S9_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 8 + 1:HDIM_ISO * 9)
const S10_IDX1 = SVector{HDIM_ISO}(HDIM_ISO * 9 + 1:HDIM_ISO * 10)
const STATE_IDX = SVector{HDIM_ISO}(1:HDIM_ISO)

# model
struct Model4 <: RD.AbstractModel
...
@inline RD.state_dim(model::Model4) = (
    ASTATE_SIZE_BASE + model.sample_state_count * CHUNK_SIZE
)
@inline RD.control_dim(model::Model4) = ACONTROL_SIZE
...
@inline sample_idxs(model::Model4) = [
    astate_sample_inds(i, j)
    for i = 1:model.sample_state_count
    for j = 1:SAMPLES_PER_STATE
]

function unscented_transform(model::Model4, astate::AbstractVector,
                             dt::Real, i::Int, camp::Real, namp::Real,
                             h_prop::AbstractMatrix)
    # get states
    offset = ASTATE_SIZE_BASE + (i - 1) * CHUNK_SIZE
    s1 = astate[offset + S1_IDX1]
    s2 = astate[offset + S2_IDX1]
    s3 = astate[offset + S3_IDX1]
    s4 = astate[offset + S4_IDX1]
    s5 = astate[offset + S5_IDX1]
    s6 = astate[offset + S6_IDX1]
    s7 = astate[offset + S7_IDX1]
    s8 = astate[offset + S8_IDX1]
    s9 = astate[offset + S9_IDX1]
    s10 = astate[offset + S10_IDX1]
    # propagate states
    s1 = h_prop * s1
    s2 = h_prop * s2
    s3 = h_prop * s3
...
# dynamics
function RD.discrete_dynamics(::Type{RK3}, model::Model4, astate::SVector,
                              acontrol::SVector, time::Real, dt::Real) where {T}
    # base dynamics
    camp = astate[CONTROLS_IDX4[1]]
    h_prop = exp(dt * (FQ_NEGI_H0_ISO + camp * NEGI_H1_ISO))
    state1 = h_prop * astate[STATE1_IDX4]
    state2 = h_prop * astate[STATE2_IDX4]
    # state3 = h_prop * astate[STATE3_IDX]
    # state4 = h_prop * astate[STATE4_IDX]
    intcontrols = astate[INTCONTROLS_IDX4[1]] + dt * astate[CONTROLS_IDX4[1]]
    controls = astate[CONTROLS_IDX4[1]] + dt * astate[DCONTROLS_IDX4[1]]
    dcontrols = astate[DCONTROLS_IDX4[1]] + dt * acontrol[D2CONTROLS_IDX4[1]]
...
function RD.discrete_dynamics(::Type{RK3}, model::Model4, z::AbstractKnotPoint)
...
function run_traj(;gate_type=xpiby2, evolution_time=60., solver_type=altro,
                  ...
                  sample_states=[IS1_ISO_], nominal_idxs=[STATE1_IDX4], static=true)
    Random.seed!(seed)
    namp = namp * dt_inv
    (sample_state_count,) = size(sample_states)
    S = Diagonal(SVector{HDIM_ISO}(fill(qs[5], HDIM_ISO)))
    model = Model4(S, nominal_idxs, namp, wnamp_cov, alpha, sample_state_count)
    ...
    x0[STATE1_IDX4] = IS1_ISO_
    x0[STATE2_IDX4] = IS2_ISO_
    # x0[STATE3_IDX] = IS3_ISO_
    # x0[STATE4_IDX] = IS4_ISO_
    xf[STATE1_IDX4] = gate * IS1_ISO_
    xf[STATE2_IDX4] = gate * IS2_ISO_
    ...
    x_max = fill(Inf, n)
    x_max[CONTROLS_IDX4] .= MAX_CONTROL_NORM_0
    x_min = fill(-Inf, n)
    x_min[CONTROLS_IDX4] .= -MAX_CONTROL_NORM_0
    if static
    ...
    x_max_boundary[CONTROLS_IDX4] .= 0
    x_min_boundary = fill(-Inf, n)
    x_min_boundary[CONTROLS_IDX4] .= 0
    if static
    ...
    # cost function
    Q = zeros(n)
    Q[STATE1_IDX4] = Q[STATE2_IDX4] = fill(qs[1], HDIM_ISO)
    Q[INTCONTROLS_IDX4] = fill(qs[2], CONTROL_COUNT)
    Q[CONTROLS_IDX4] = fill(qs[3], CONTROL_COUNT)
    Q[DCONTROLS_IDX4] = fill(qs[4], CONTROL_COUNT)
    ...
    R = zeros(m)
    R[D2CONTROLS_IDX4] = fill(qs[6], CONTROL_COUNT)
    if static
        Q = SVector{n}(Q)
        R = SVector{m}(R)
    end
    ...
    target_astate_constraint = GoalConstraint(xf, [STATE1_IDX4; STATE2_IDX4; INTCONTROLS_IDX4])
    # must obey unit norm
    norm_idxs = copy(sample_idxs(model))
    push!(norm_idxs, STATE1_IDX4)
    push!(norm_idxs, STATE2_IDX4)
    ...
    cidx_arr = Array(CONTROLS_IDX4)
    d2cidx_arr = Array(D2CONTROLS_IDX4)
