...
# state indices
const STATE1_IDX = SVector{HDIM_ISO}(1:HDIM_ISO)  #was displaying error here hence commented
...
const INTCONTROLS_IDX2 = SVector{CONTROL_COUNT}(STATE3_IDX[end] + 1:STATE3_IDX[end] + CONTROL_COUNT)
const CONTROLS_IDX2 = SVector{CONTROL_COUNT}(INTCONTROLS_IDX2[end] + 1:
                                            INTCONTROLS_IDX2[end] + CONTROL_COUNT)
const DCONTROLS_IDX2 = SVector{CONTROL_COUNT}(CONTROLS_IDX2[end] + 1:
                                             CONTROLS_IDX2[end] + CONTROL_COUNT)
# control indices
const D2CONTROLS_IDX2 = SVector{CONTROL_COUNT}(1:CONTROL_COUNT)
# sample indices
const S1_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 0 + 1:HDIM_ISO * 1)
const S2_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 1 + 1:HDIM_ISO * 2)
const S3_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 2 + 1:HDIM_ISO * 3)
const S4_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 3 + 1:HDIM_ISO * 4)
const S5_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 4 + 1:HDIM_ISO * 5)
const S6_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 5 + 1:HDIM_ISO * 6)
const S7_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 6 + 1:HDIM_ISO * 7)
const S8_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 7 + 1:HDIM_ISO * 8)
const S9_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 8 + 1:HDIM_ISO * 9)
const S10_IDX2 = SVector{HDIM_ISO}(HDIM_ISO * 9 + 1:HDIM_ISO * 10)
const STATE_IDX = SVector{HDIM_ISO}(1:HDIM_ISO)

# model
struct Model1 <: RD.AbstractModel
...
@inline RD.state_dim(model::Model1) = (
    ASTATE_SIZE_BASE + model.sample_state_count * CHUNK_SIZE
)
@inline RD.control_dim(model::Model1) = ACONTROL_SIZE
...
@inline sample_idxs(model::Model1) = [
    astate_sample_inds(i, j)
    for i = 1:model.sample_state_count
    for j = 1:SAMPLES_PER_STATE
]

function unscented_transform(model::Model1, astate::AbstractVector,
                             negi_hc::AbstractMatrix, h_prop::AbstractMatrix,
                             dt::Real, i::Int)
    # get states
    offset = ASTATE_SIZE_BASE + (i - 1) * CHUNK_SIZE
    s1 = astate[offset + S1_IDX2]
    s2 = astate[offset + S2_IDX2]
    s3 = astate[offset + S3_IDX2]
    s4 = astate[offset + S4_IDX2]
    s5 = astate[offset + S5_IDX2]
    s6 = astate[offset + S6_IDX2]
    s7 = astate[offset + S7_IDX2]
    s8 = astate[offset + S8_IDX2]
    s9 = astate[offset + S9_IDX2]
    s10 = astate[offset + S10_IDX2]
    # grab chol info
...
# dynamics
function RD.discrete_dynamics(::Type{RK3}, model::Model1, astate::StaticVector,
                              acontrol::StaticVector, time::Real, dt::Real) where {n}
    # base dynamics
    negi_hc = astate[CONTROLS_IDX2[1]] * NEGI_H1_ISO
    ...
    intcontrols = astate[INTCONTROLS_IDX2] + dt * astate[CONTROLS_IDX2]
    controls = astate[CONTROLS_IDX2] + dt * astate[DCONTROLS_IDX2]
    dcontrols = astate[DCONTROLS_IDX2] + dt * acontrol[D2CONTROLS_IDX2]
...
function run_traj(;gate_type=xpiby2, evolution_time=60., solver_type=altro,
    ...
    model = Model1(S, nominal_idxs, fq_cov, alpha, sample_state_count)
    ...
    x_max = fill(Inf, n)
    x_max[CONTROLS_IDX2] .= MAX_CONTROL_NORM_0
    x_min = fill(-Inf, n)
    x_min[CONTROLS_IDX2] .= -MAX_CONTROL_NORM_0
    if static
    ...
    x_max_boundary[CONTROLS_IDX2] .= 0
    x_min_boundary = fill(-Inf, n)
    x_min_boundary[CONTROLS_IDX2] .= 0
    if static
    ...
    Q[INTCONTROLS_IDX2] = fill(qs[2], CONTROL_COUNT)
    Q[CONTROLS_IDX2] = fill(qs[3], CONTROL_COUNT)
    Q[DCONTROLS_IDX2] = fill(qs[4], CONTROL_COUNT)
    ...
    R = zeros(m)
    R[D2CONTROLS_IDX2] = fill(qs[6], CONTROL_COUNT)
    if static
        Q = SVector{n}(Q)
        R = SVector{m}(R)
    end
    ...
    target_astate_constraint = GoalConstraint(xf, [STATE1_IDX; STATE2_IDX; INTCONTROLS_IDX2])
    # must obey unit norm
    ...
    cidx_arr = Array(CONTROLS_IDX2)
    d2cidx_arr = Array(D2CONTROLS_IDX2)
